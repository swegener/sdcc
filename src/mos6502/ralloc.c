...
{
  bitVect *rmask = newBitVect (m6502_nRegs);

  /* do the special cases first */
  if (ic->op == IFX)
    {
      rmask = bitVectUnion (rmask, m6502_rUmaskForOp (IC_COND (ic)));
      goto ret;
    }
  ...
  if (ic->op == JUMPTABLE)
    {
      rmask = bitVectUnion (rmask, m6502_rUmaskForOp (IC_JTCOND (ic)));
      goto ret;
    }

  /* of all other cases */
  if (IC_LEFT (ic))
    rmask = bitVectUnion (rmask, m6502_rUmaskForOp (IC_LEFT (ic)));

  if (IC_RIGHT (ic))
    rmask = bitVectUnion (rmask, m6502_rUmaskForOp (IC_RIGHT (ic)));

  if (IC_RESULT (ic))
    rmask = bitVectUnion (rmask, m6502_rUmaskForOp (IC_RESULT (ic)));
      ...
      for (ic = ebbs[i]->sch; ic; ic = ic->next)
        {
          ...
          ic->rMask = newBitVect (m6502_nRegs + 1);
...
packRegsForOneuse (iCode * ic, operand * op, eBBlock * ebp)
{
  ...
  /* only up to 2 bytes */
  if (getSize (operandType (op)) > (fReturnSizeM6502 - 2))
    return NULL;
...
packRegsForAccUse (iCode * ic)
{
  ...
  if ((POINTER_GET(uic))
    ...
    {
      OP_SYMBOL (IC_RESULT (ic))->accuse = ACCUSE_YX;
      return;
    }
  #endif
...
replaceAccuseOperand (operand * op)
{
  ...
  wassert ((sym->accuse == ACCUSE_XA) || (sym->accuse == ACCUSE_YX));
  wassert (sym->nRegs <= 2);

  if (sym->accuse == ACCUSE_XA)
    {
      sym->regs[0] = m6502_reg_a;
      if (sym->nRegs > 1)
        sym->regs[1] = m6502_reg_x;
    }
  else /* must be sym->accuse == ACCUSE_YX */
    {
      sym->regs[0] = m6502_reg_x;
      if (sym->nRegs > 1)
        sym->regs[1] = m6502_reg_y;
    }  
  sym->accuse = 0;
}
