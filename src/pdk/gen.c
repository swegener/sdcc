...
cheapMove (const asmop *result, int roffset, const asmop *source, int soffset, bool a_dead, bool p_dead, bool f_dead)
{
  ...
  if (aopSame (result, roffset, source, soffset, 1))
  ...
  else if (source->type == AOP_STL)
    {
      ...
      if (G.p.type == AOP_STL && source->aopu.stk_off == G.p.offset && aopInReg (result, roffset, P_IDX))
        return;
...
genMove_o (asmop *result, int roffset, asmop *source, int soffset, int size, bool a_dead_global)
{
  ...
  if (result->type == AOP_SFR || source->type == AOP_SFR)
    switch (size)
      {
      case 1:
        cheapMove (result, roffset, source, soffset, a_dead_global, true, true);
        return;
      case 2:
  ...
  if (size == 2 && aopInReg (result, roffset, P_IDX) && aopInReg (result, roffset + 1, A_IDX) && source->type == AOP_STK)
    {
      cheapMove (result, roffset + 1, source, soffset + 1, true, true, true);
      cheapMove (result, roffset + 0, source, soffset + 0, false, true, true);
      return;
    }
    ...
    {
      cheapMove (result, roffset + 1, source, soffset + 1, a_dead_global, true, true);
      cheapMove (result, roffset + 0, source, soffset + 0, a_dead_global, true, true);
      return;
    }
  else if (size == 2 &&
      ...
      if (soffset + 1 == roffset) // Use xch via a.
      ...
      else // Copy high-to-low to avoid overwriting of still-needed bytes.
        {
          for (int i = size - 1; i >= 0; i--)
            cheapMove (result, roffset + i, source, soffset + i, true, true, true);
        }
      if (!a_dead_global)
        popAF();
      return;
...
genPlus (const iCode *ic)
{
  ...
  if (left->aop->type == AOP_LIT || aopInReg (right->aop, 0, A_IDX) || right->aop->type == AOP_STL || right->aop->type == AOP_STK && !(aopInReg (left->aop, 0, A_IDX) || left->aop->type == AOP_STL))
    {
      operand *t = right;
      right = left;
      left = t;
  ...
  for (int i = 0; i < size; i++)
    {
      ...
      if (!moved_to_a && (left->aop->type == AOP_DIR || aopInReg (left->aop, i, P_IDX)) && right->aop->type != AOP_STK && aopSame (left->aop, i, result->aop, i, 1))
        ...
        {
          if (!moved_to_a)
            cheapMove (ASMOP_A, 0, left->aop, i, true, p_dead, !started);
          cheapMove (ASMOP_P, 0, right->aop, i, false, p_dead, !started);
    ...
    for (int i = 0; i < size; i++)
      {
        if (left->aop->type == AOP_STK && aopInReg (right->aop, i, P_IDX))
          ...
          {
            cheapMove (ASMOP_A, 0, left->aop, i, true, true, true);
            if (right->aop->type == AOP_STK || right->aop->type == AOP_CODE || right->aop->type == AOP_STL)
              {
        ...
        if (ifx && i + 1 == size && ((ic->op == EQ_OP) ^ (bool)(IC_FALSE(ifx))))
          {
            if (TARGET_IS_PDK13)
              {
                symbol *tlbl = (regalloc_dry_run ? 0 : newiTempLabel (NULL));
                emit2 ("ceqsn", "a, %s", (right->aop->type == AOP_STK || right->aop->type == AOP_CODE || right->aop->type == AOP_STL) ? "p" : aopGet (right->aop, i));
                emitJP (tlbl, 0.0f);
                cost (2, 3);
                emitJP (IC_FALSE (ifx) ? IC_FALSE (ifx) : IC_TRUE (ifx), 0.0f);
                emitLabel (tlbl);
              ...
              {
                emit2 ("cneqsn", "a, %s", (right->aop->type == AOP_STK || right->aop->type == AOP_CODE || right->aop->type == AOP_STL) ? "p" : aopGet (right->aop, i));
                cost (1, 1);
                emitJP (IC_FALSE (ifx) ? IC_FALSE (ifx) : IC_TRUE (ifx), 0.0f);
              }
          }
  ...
  for (int i = 0; i < size; i++)
    {
      ...
      if (aopIsLitVal (right->aop, i, 1, 0xff))
        ...
        cheapMove (result->aop, i, ASMOP_ZERO, 0, true, p_dead, true);
      else if ((left->aop->type == AOP_SFR || aopInReg (left->aop, i, P_IDX)) && aopSame (left->aop, i, result->aop, i, 1) && bit >= 0)
        {
          ...
          for (int i = offset; i < size; i++)
            {
              if (result->aop->type == AOP_STK)
                {
                  cheapMove (ASMOP_A, 0, result->aop, i, true, true, i <= offset);
                  emit2 ((i > offset) ? "slc" : "sl", "a");
                  cost (1, 1);
                  cheapMove (result->aop, i, ASMOP_A, 0, true, true, i + 1 != size);
                }
              else
                {
      ...
      for(int i = 0; i < size; i++)
        {
          if (result->aop->type == AOP_STK && size == 1)
            {
              pushAF ();
              cheapMove (ASMOP_A, 0, result->aop, i, true, true, true);
              emit2("sl", "a");
              cost (1, 1);
              cheapMove (result->aop, i, ASMOP_A, 0, true, true, true);
              popAF ();
            ...
            {
              cheapMove (ASMOP_P, 0, result->aop, i, false, true, !i);
              emit2(i ? "slc" : "sl", "p");
              cost (1, 1);
              cheapMove (result->aop, i, ASMOP_P, 0, false, true, i + 1 == size);
            }
          else
            {
      ...
      for(int i = size - 2; i >= 0; i--)
        {
          if (result->aop->type == AOP_STK)
            {
              cheapMove (ASMOP_P, 0, result->aop, i, true, true, false);
              emit2 ("src", "p");
              cheapMove (result->aop, i, ASMOP_P, 0, true, true, i == 1);
            }
          else
            {
              emit2 ("src", "%s", aopGet (result->aop, i));
  ...
  if (right->aop->type == AOP_DUMMY)
    {
      ...
      for (i = 0; i < result->aop->size; i++)
        cheapMove (result->aop, i, ASMOP_A, 0, false, true, true);
    }
