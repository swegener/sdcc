...
noi2sym (char *filename)
{
  ...
  while ((read = getline(&line, &line_len, noi)) != -1) {
    ...
    if(strcmp(line, "DEF") == 0 && ((strcmp(line+4, "s__BSS") == 0) || strcmp(line+4, "s__HEAP") == 0
       || (strncmp(line+4, "s__", 3) != 0 && strncmp(line+4, "l__", 3) != 0 && strncmp(line+4, ".__", 3) != 0
           && strncmp(line+4, "C$", 2) != 0 && strncmp(line+4, "G$", 2) != 0 && strncmp(line+4, "S$", 2) != 0
           && strncmp(line+4, "XG$", 3)&& strncmp(line+4, "b_", 2) != 0)))
      {
        address = strtok(NULL, " ");
        if (!address)
