/** A test based on the benchmark and its self-test
    It is based on Coremark 1.01 by Shay Gal-On.
 */

/* Coremark 1.01 is licensed under Apache 2.0
*/

#include <testfwk.h>

#if !defined(__SDCC_pdk13) && !defined(__SDCC_pdk14) && !defined(__SDCC_pdk15) && !defined(__SDCC_pdk16) && !defined(__SDCC_pic14) && (!defined(__SDCC_mcs51) || (defined(__SDCC_MODEL_LARGE) || defined(__SDCC_MODEL_HUGE)) && defined(__SDCC_STACK_AUTO)) && !defined(__SDCC_hc08) && !defined(__SDCC_s08) && !defined(__SDCC_mos6502) && !defined(__SDCC_mos65c02) && !defined(__SDCC_z80) && !defined(__SDCC_z180) && !defined(__SDCC_ds390) /* We need at least 2.5 KB of memory, which no pdk port has; for msc51, hc08, s08, mos6502, mos65c02 we get a DSEG length error - i.e. too much memory used for local variables in __near - this should improve when we have Chaitin-style allocation there (RFE #958), any maybe some other optimizations - z80, z180, ds390: fails, possibly a bug in codegen */
...
// core_list_join.c content below
...
typedef ee_s32(*list_cmp)(list_data *a, list_data *b, core_results *res) __reentrant; /* Deviation from benchmark: some ports (hc08, s08) need the __reentrant. */
...
*/
ee_s32 cmp_complex(list_data *a, list_data *b, core_results *res) __reentrant { /* Deviation from benchmark: some ports (hc08, s08) need the __reentrant. */
...
/* Function: cmp_idx
	...
	Can be used by mergesort.
*/
ee_s32 cmp_idx(list_data *a, list_data *b, core_results *res) __reentrant { /* Deviation from benchmark: some ports (hc08, s08) need the __reentrant. */
	if (res==NULL) {
...
#error "Please define a way to initialize a memory block."
	...
	/* deviation from benchmark - we want only one iteration for regression testing */
	results[0].iterations = 1;
	...
	if (total_errors<0)
        ...
        /* deviation from benchmark: report errors to regression test infrastructure */
	ASSERT (!total_errors);
...
#define GETMYTIME(_t) (*_t=0) /* deviation from benchmark: we just want the self-test, don't care about time */
...
*/
secs_ret time_in_secs(CORE_TICKS ticks) {
	return 10; /* deviation from benchmark - just afixed value to not trigger error detection, no matter how muchtimeit actually took */
}
